...
class TransactionDevSerializer(TopupSerializer, serializers.Serializer):
    id = serializers.IntegerField(read_only=True)
    trx_code = serializers.CharField(read_only=True)
    product = ProductDetailSerializer(read_only=True)
    timestamp = serializers.DateTimeField(read_only=True)
    closed = serializers.BooleanField(read_only=True)
    buyer = serializers.PrimaryKeyRelatedField(queryset=User.objects.all(), write_only=True)
    t_notive = serializers.BooleanField(read_only=True)
    ...
    def create(self, validated_data):
        ...
        product_code = validated_data.get('product_code')
        product_obj = Product.objects.get(product_code=product_code)
        transaction_obj = TrxDeveloper(
            product_obj, phone, validated_data.get('buyer')
        )
        
        return transaction_obj


class TrxDeveloper(object):
    def __init__(self, product, phone, buyer):
        self.phone = phone
        self.product = product
        self.buyer = buyer
        self.status = 'IN'
        self.id = 876
        self.trx_code = '1288713131'
